...
/* isCurrentThreadInterrupted == isInterrupted
*
* threadName== getName()
* */
...
public class Solution
{
    ...
    {
        // вот здесь создали объект класса Water, и уже это будет единственный объект!
        Water water = new Water("water");

        for (int i = 0; i < countThreads; i++)
        {
            threads.add(new Thread(water, "#" + i)); // в этом цикле мы наполняем лист из Thread-ов
        }

        for (int i = 0; i < countThreads; i++)
        {
            threads.get(i).start();                  // здесь, мы получаем каждый из thread-ов и стартуем
        }
    }
